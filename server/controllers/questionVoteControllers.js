...
var deleteVote = function (req,res) {
  ...
  .then(function (findVote) {
    if (!findVote) {
      res.send({msg: `Vote with voter_id ${req.decoded.id} and question_id ${req.params.question_id} not found`})
    } else {
      findVote.destroy()
      .then(function () {
        res.send({msg: `Vote with voter_id ${req.decoded.id} and question_id ${req.params.question_id} deleted`})
      })
      .catch(function (err) {
        res.status(500).send(err)
      })
